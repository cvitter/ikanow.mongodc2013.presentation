...
(((window, undefined) => {
    ...
    var // Map over the $ in case of overwrite
    _$ = window.$;
    ...
    var // Define a local copy of jQuery
    jQuery = (selector, context) => // The jQuery object is actually just the init constructor 'enhanced'
    new jQuery.fn.init(selector, context, rootjQuery);

    var // Used for matching numbers
    core_pnum = /[\-+]?(?:\d*\.|)\d+(?:[eE][\-+]?\d+|)/.source;

    var // Used for detecting and trimming whitespace
    core_rnotwhite = /\S/;

    var core_rspace = /\s+/;

    var // Make sure we trim BOM and NBSP (here's looking at you, Safari 5.0 and IE)
    rtrim = /^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g;
    ...
    var // Matches dashed string for camelizing
    rmsPrefix = /^-ms-/;

    var rdashAlpha = /-([\da-z])/gi;

    var // Used by jQuery.camelCase as callback to replace()
    fcamelCase = (all, letter) => ( letter + "" ).toUpperCase();

    var // The ready event handler and self cleanup method
    DOMContentLoaded = () => {
    ...
    jQuery.fn = jQuery.prototype = {
        ...
        slice(...args) {
            return this.pushStack( core_slice.apply( this, args ),
                "slice", core_slice.call(args).join(",") );
        },
    ...
    jQuery.extend({
        ...
        isArray: Array.isArray || (obj => jQuery.type(obj) === "array"),
        ...
        each(obj, callback, args) {
            ...
            var isObj = length === undefined || jQuery.isFunction( obj );
        ...
        map(elems, callback, arg) {
            ...
            var // jquery objects are treated as arrays
        ...
        access(elems, fn, key, value, chainable, emptyGet, pass) {
            var exec;
            var bulk = key == null;
            var i = 0;
            var length = elems.length;
            ...
            // Sets one value
                ...
                if ( bulk ) {
                    ...
                    if ( exec ) {
                        exec = fn;
                        fn = (elem, key, value) => exec.call( jQuery( elem ), value );
    ...
    function createOptions( options ) {
        ...
        jQuery.each( options.split( core_rspace ), (_, flag) => {
            object[ flag ] = true;
        });
        return object;
    }
    ...
    jQuery.Callbacks = options => {
        ...
        var // Flag to know if list was already fired
        fired;

        var // Flag to know if list is currently firing
        firing;

        var // First callback to fire (used internally by add and fireWith)
        firingStart;

        var // End of the loop when firing
        firingLength;

        var // Index of currently firing callback (modified by remove if needed)
        firingIndex;

        var // Actual callback list
        list = [];

        var // Stack of fire calls for repeatable lists
        stack = !options.once && [];
    ...
    jQuery.extend({
        ...
        Deferred(func) {
            ...
            var promise = {
				...
				then(...args) /* fnDone, fnFail, fnProgress */ {
					var fns = args;
        ...
        when(subordinate /* , ..., subordinateN */) {
            ...
            var // the count of uncompleted subordinates
            remaining = length !== 1 || ( subordinate && jQuery.isFunction( subordinate.promise ) ) ? length : 0;

            var // the master Deferred. If resolveValues consist of only a single Deferred, just use that.
            deferred = remaining === 1 ? subordinate : jQuery.Deferred();

            var // Update function for both resolve and progress values
    ...
    jQuery.support = ((() => {
        ...
        jQuery(() => {
            ...
            var tds;
            var marginDiv;
            var divReset = "padding:0;margin:0;border:0;display:block;overflow:hidden;";
    ...
    jQuery.extend({
        ...
        data(elem, name, data, pvt /* Internal Use Only */) {
            ...
            var // We have to handle DOM nodes and JS objects differently because IE6-7
            // can't GC object references properly across the DOM-JS boundary
            isNode = elem.nodeType;

            var // Only DOM nodes need the global jQuery cache; JS object data is
            // attached directly to the object so GC can occur automatically
            cache = isNode ? jQuery.cache : elem;

            var // Only defining an ID for JS objects if its cache already exists allows
        ...
        removeData(elem, name, pvt /* Internal Use Only */) {
            if ( !jQuery.acceptData( elem ) ) {
                return;
            }
            ...
            var // See jQuery.data for more information
            cache = isNode ? jQuery.cache : elem;
        ...
        toggleClass(value, stateVal) {
            ...
            return this.each(function() {
                if ( type === "string" ) {
                    ...
                    var self = jQuery( this );
                    var state = stateVal;
                    var classNames = value.split( core_rspace );
    ...
    var hoverHack = events => jQuery.event.special.hover ? events : events.replace( rhoverHack, "mouseenter$1 mouseleave$1" );
    ...
    (((window, undefined) => {
        ...
        var // Use a stripped-down indexOf if a native one is unavailable
        ...
        var // Augment a function for special use by Sizzle
        markFunction = (fn, value) => {
        ...
        var // Loosely modeled on CSS identifier characters
        // An unquoted value should be a CSS identifier (http://www.w3.org/TR/css3-selectors/#attribute-selectors)
        // Proper syntax: http://www.w3.org/TR/CSS21/syndata.html#value-def-identifier
        identifier = characterEncoding.replace( "w", "w#" );

        var // Acceptable operators http://www.w3.org/TR/selectors/#attribute-selectors
        operators = "([*^$|!~]?=)";

        var attributes = "\\[" + whitespace + "*(" + characterEncoding + ")" + whitespace +
            "*(?:" + operators + whitespace + "*(?:(['\"])((?:\\\\.|[^\\\\])*?)\\3|(" + identifier + ")|)|)" + whitespace + "*\\]";

        var // Prefer arguments not in parens/brackets,
        ...
        var // For matchExpr.POS and matchExpr.needsContext
        pos = ":(even|odd|eq|gt|lt|nth|first|last)(?:\\(" + whitespace +
            "*((?:-\\d)?\\d*)" + whitespace + "*\\)|)(?=[^-]|$)";

        var // Leading and non-escaped trailing whitespace, capturing some non-whitespace characters preceding the latter
        ...
        var // Easily-parseable/retrievable ID or TAG or CLASS selectors
        rquickExpr = /^(?:#([\w\-]+)|(\w+)|\.([\w\-]+))$/;

        var rnot = /^:not/;
        var rsibling = /[\x20\t\r\n\f]*[+~]/;
        var rendsWithNot = /:not\($/;
        var rheader = /h\d/i;
        ...
        var // Check if getElementsByTagName("*") returns only elements
        assertTagNameNoComments = assert(div => {
            div.appendChild( document.createComment("") );
            return !div.getElementsByTagName("*").length;
        });

        var // Check if getAttribute returns normalized href attributes
        ...
        var // Check if attributes should be retrieved by attribute nodes
        ...
        var // Check if getElementsByClassName can be trusted
        ...
        var // Check if getElementById returns elements by name
        // Check if getElementsByName privileges form controls or returns elements by ID
        assertUsableName = assert(div => {
            // Inject content
            div.id = expando + 0;
        ...
        Sizzle.matches = (expr, elements) => Sizzle( expr, null, null, elements );

        Sizzle.matchesSelector = (elem, expr) => Sizzle( expr, null, null, [ elem ] ).length > 0;
        ...
        contains = Sizzle.contains = docElem.contains ?
            ...
            (a, b) => b && !!( a.compareDocumentPosition( b ) & 16 ) :
            (a, b) => {
        ...
        Expr = Sizzle.selectors = {
            ...
            find: {
                ...
                "NAME": assertUsableName && ((tag, context) => {
                    if ( typeof context.getElementsByName !== strundefined ) {
                        return context.getElementsByName( name );
                    }
                }),

                "CLASS": assertUsableClassName && ((className, context, xml) => {
            ...
            filter: {
                "ID": assertGetIdNotName ?
                    id => {
                        ...
                        return elem => elem.getAttribute("id") === id;
                    } :
                    id => {
                        id = id.replace( rbackslash, "" );
                        return elem => {
                ...
                "CLASS": function( className ) {
                    ...
                    return elem => pattern.test( elem.className || (typeof elem.getAttribute !== strundefined && elem.getAttribute("class")) || "" );
                },
            ...
            pseudos: {
                ...
                "contains": markFunction(text => elem => ( elem.textContent || elem.innerText || getText( elem ) ).indexOf( text ) > -1),
                ...
                // Positional types
                "first": createPositionalPseudo((matchIndexes, length, argument) => [ 0 ]),

                "last": createPositionalPseudo((matchIndexes, length, argument) => [ length - 1 ]),

                "eq": createPositionalPseudo((matchIndexes, length, argument) => [ argument < 0 ? argument + length : argument ]),

                "even": createPositionalPseudo((matchIndexes, length, argument) => {
                ...
                "odd": createPositionalPseudo((matchIndexes, length, argument) => {
        ...
        function tokenize( selector, parseOnly ) {
            ...
            var groups;
            var preFilters;
            var cached = tokenCache[ expando ][ selector ];
        ...
        function addCombinator( matcher, combinator, base ) {
            ...
            var checkNonElements = base && combinator.dir === "parentNode";
            var doneName = done++;
            ...
            return combinator.first ?
                ...
                (elem, context, xml) => {
                    ...
                    if ( !xml ) {
                        ...
                        var cachedkey = dirkey + cachedruns;
                        while ( (elem = elem[ dir ]) ) {
                            if ( checkNonElements || elem.nodeType === 1 ) {
        ...
        function setMatcher( preFilter, selector, matcher, postFilter, postFinder, postSelector ) {
            ...
            return markFunction((seed, results, context, xml) => {
                ...
                var // Get initial elements from seed or context
                elems = seed || multipleContexts( selector || "*", context.nodeType ? [ context ] : context, [], seed );

                var // Prefilter to get matcher input, preserving a map for seed-results synchronization
        ...
        function matcherFromTokens( tokens ) {
            ...
            var // The foundational matcher ensures that elements are reachable from top-level context(s)
            matchContext = addCombinator( elem => elem === checkContext, implicitRelative, true );

            var matchAnyContext = addCombinator( elem => indexOf.call( checkContext, elem ) > -1, implicitRelative, true );

            var matchers = [ (elem, context, xml) => ( !leadingRelative && ( xml || context !== outermostContext ) ) || (
        ...
        function matcherFromGroupMatchers( elementMatchers, setMatchers ) {
            ...
            var superMatcher = (seed, context, xml, results, expandContext) => {
                var elem, j, matcher,
                    setMatched = [],
                    matchedCount = 0,
                    i = "0",
        ...
        compile = Sizzle.compile = (selector, group /* Internal Use Only */) => {
            var i;
            var setMatchers = [];
            var elementMatchers = [];
            var cached = compilerCache[ expando ][ selector ];
        ...
        if ( document.querySelectorAll ) {
            ((() => {
                ...
                var // qSa(:focus) reports false when true (Chrome 21),
                // A support test would require too much code (would include document ready)
                rbuggyQSA = [":focus"];

                var // matchesSelector(:focus) reports false when true (Chrome 21),
                ...
                select = (selector, context, results, seed, xml) => {
                    ...
                    if ( !seed && !xml && (!rbuggyQSA || !rbuggyQSA.test( selector )) ) {
                        ...
                        var newSelector = context.nodeType === 9 && selector;
    ...
    var // methods guaranteed to produce a unique set when starting from a unique set
    guaranteedUnique = {
		children: true,
		contents: true,
		next: true,
    ...
    function winnow( elements, qualifier, keep ) {
        ...
        if ( jQuery.isFunction( qualifier ) ) {
            ...
            var filtered = jQuery.grep(elements, elem => elem.nodeType === 1);
    ...
    var // checked="checked" or checked
    ...
    jQuery.fn.extend({
        ...
        append(...args) {
            return this.domManip(args, true, function( elem ) {
        ...
        before(...args) {
            if ( !isDisconnected( this[0] ) ) {
                return this.domManip(args, false, function( elem ) {
                    this.parentNode.insertBefore( elem, this );
                });
            }

            if ( args.length ) {
                var set = jQuery.clean( args );
    ...
    jQuery.extend({
        ...
        clean(elems, context, fragment, scripts) {
            ...
            var handleScript;
            var jsTags;
            var safe = context === document && safeFragment;
            var ret = [];
    ...
    var eventsToggle = jQuery.fn.toggle;
    ...
    jQuery.fn.extend({
        css(name, value) {
            return jQuery.access( this, (elem, name, value) => value !== undefined ?
    ...
    function getWidthOrHeight( elem, name, extra ) {
        ...
        var isBorderBox = jQuery.support.boxSizing && jQuery.css( elem, "boxSizing" ) === "border-box";
    ...
    if ( jQuery.expr && jQuery.expr.filters ) {
        jQuery.expr.filters.hidden = elem => ( elem.offsetWidth === 0 && elem.offsetHeight === 0 ) || (!jQuery.support.reliableHiddenOffsets && ((elem.style && elem.style.display) || curCSS( elem, "display" )) === "none");
    ...
    jQuery.each({
        ...
        jQuery.cssHooks[ prefix + suffix ] = {
            expand(value) {
                ...
                var // assumes a single number if not a string
    ...
    var // IE leaves an \r character at EOL
    rheaders = /^(.*?):[ \t]*([^\r\n]*)\r?$/mg;

    var // #7653, #8125, #8152: local protocol detection
    ...
    var // Avoid comment-prolog char sequence (#10098); must appease lint and evade compression
    allTypes = ["*/"] + ["*"];
    ...
    jQuery.extend({
        ...
        ajax(url, options) {
            ...
            var // To know if global events are to be dispatched
            fireGlobals;
            ...
            var // Status-dependent callbacks
            statusCode = s.statusCode || {};

            var // Headers (they are sent all at once)
            requestHeaders = {};
            ...
            if ( !s.hasContent ) {
                ...
                if ( s.cache === false ) {
                    ...
                    var // try replacing _= if it is there
    ...
    function ajaxConvert( s, response ) {
        ...
        var // Work with a copy of dataTypes in case we need to modify it for conversion
    ...
    jQuery.ajaxPrefilter( "json jsonp", (s, originalSettings, jqXHR) => {
        ...
        var data = s.data;
        var url = s.url;
        var hasCallback = s.jsonp !== false;
        var replaceInUrl = hasCallback && rjsonp.test( url );

        var replaceInData = hasCallback && !replaceInUrl && typeof data === "string" &&
    ...
    function Animation( elem, properties, options ) {
        ...
        var deferred = jQuery.Deferred().always( () => {
    ...
    jQuery.each( {scrollLeft: "pageXOffset", scrollTop: "pageYOffset"}, (method, prop) => {
        ...
        jQuery.fn[ method ] = function( val ) {
            return jQuery.access( this, (elem, method, val) => {
